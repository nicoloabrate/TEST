...
class Diffusion():
    ...
    def __init__(self, geom, steady, prod=None, BC=True, fmt='csr',
        ...
        if allope is True:
            self.Fp = MG.promptfiss(geom, angular_model, fmt=fmt, adjoint=True)
            self.Fd = MG.delfiss(geom, angular_model, fmt=fmt, adjoint=True)
            self.F = MG.fission(geom, angular_model, fmt=fmt, adjoint=True)
