...
switch geom_type
    ...
    case 1 % slab
        ...
        for ii=1:N_reg
            ...
            f_int(ii)    = trapz(x,f_handle{ii}(x));           % integrate the function
    ...
    case 2 % sphere
        for ii=1:N_reg
            ...
            x(ii,:)    = linspace(t_reg(ii),t_reg(ii+1),NP);    % grid definition
            f_handle{ii} = eval(sprintf('@(x) %s',f_def{ii}));  % build function handle
            f_int(ii)    = trapz(x,f_handle{ii}(x)*4*pi.*x.^2); % integrate the function
            f_shape{ii}  = eval(sprintf('@(x) (%s)*(%f)/(%f)',f_def{ii},V(ii),f_int(ii)));
    ...
    case 3 % cylinder
        for ii=1:N_reg
            ...
            x(ii,:)    = linspace(t_reg(ii),t_reg(ii+1),NP);   % grid definition
            f_handle{ii} = eval(sprintf('@(x) %s',f_def{ii})); % build function handle
            f_int(ii)    = trapz(x,f_handle{ii}(x)*2*pi.*x);   % integrate the function
            f_shape{ii}  = eval(sprintf('@(x) (%s)*(%f)/(%f)',f_def{ii},V(ii),f_int(ii)));
