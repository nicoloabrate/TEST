...
slab2 = Slab(M, xlayers, matname, ['MarkOdd'], G)
...
for n in range(1, Nmax):
    ...
    if n % 2 == 0:
        print('PN=%g' % n)
        PN = NTE.PN(slab2, n, fmt='csc', allope=True)
        k = kappa(slab, PN, nev=nev, verbosity=True)
        g = gamma(slab, PN, nev=nev, verbosity=True)
        a = alphaprompt(slab, PN, nev=nev, algo='eig', verbosity=True)
        kappa0_even.append(k.eigvals[0])
        gamma0_even.append(g.eigvals[0])
        a0 = np.min(abs(a.eigvals[a.eigvals.imag == 0].real))
        pos = np.where(abs(a.eigvals) == a0)
        alpha0_even.append(np.asscalar(a.eigvals[abs(a.eigvals) == a0, ]))


kappa0 = np.asarray(kappa0).real
gamma0 = np.asarray(gamma0).real
alpha0 = np.asarray(alpha0).real

h5.writeh5([kappa0, gamma0, alpha0], 'eigvals_%s_%g.h5' % (bc, M))

dk = 1e5*(kappa0-k_ref).real
...
ax = plt.gca()
ax.yaxis.grid(True)
b1 = plt.bar(range(1, Nmax, 2), da[0::2], color='midnightblue',
             width=1, edgecolor='black', align='center')
handles.append(b1)
